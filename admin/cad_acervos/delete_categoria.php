...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $deleteQuery = "DELETE FROM cad_categoria WHERE id = '$id'";
    $result = $mysqli->exec($deleteQuery);

    if ($result) {
        echo json_encode(["status" => "success", "message" => "Cadastro excluído com sucesso!"]);
    } else {
        echo json_encode(["status" => "error", "message" => "Erro ao excluir cadastro."]);
    }
} else {
